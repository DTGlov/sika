-- Step 1: Rename the column on profiles
ALTER TABLE profiles RENAME COLUMN future_percent TO savings_percent;

-- Step 2: Update the handle_new_user trigger function — corrected version
...
BEGIN
  ...
  -- Default categories (NO Savings expense categories — money INTO savings = transfer)
  INSERT INTO categories (user_id, bucket_id, name, icon, is_default) VALUES
    ...
    -- Income presets (no bucket — full list of 8)
  ...
  -- Default accounts (note: BOTH `type` and `account_type` columns must be set)
