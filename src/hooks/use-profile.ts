...
export function useProfile() {
  const { user, profile, setProfile, setIncomeSources, setAccounts, setDismissedHints, setStreaks, setMomentum } = useAuthStore();
  const supabase = createClient();

  const fetchProfile = useCallback(async () => {
    if (!user) return;
    const [profileRes, sourcesRes, accountsRes, hintsData, streaksRes, momentumRes] = await Promise.all([
  ...
  }, [user, supabase, setProfile, setIncomeSources, setAccounts, setDismissedHints, setStreaks, setMomentum]);
