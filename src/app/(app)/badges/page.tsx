    ...
    <div className="max-w-2xl mx-auto pb-24">
      ...
      <div className="sticky top-0 z-10 bg-background border-b border-card">
        <div className="flex items-center gap-3 px-4 h-14">
          ...
          <h1 className="text-foreground font-semibold text-base">Your Badges</h1>
        </div>
      </div>
      ...
      <div className="px-4 md:px-8 pt-6 space-y-8">
        ...
        <p className="text-sm text-muted-foreground">
          Earned:{' '}
          <span className="text-foreground font-semibold">{earned.length}</span>
          {' '}of{' '}
          <span className="text-foreground font-semibold">{allBadges.length}</span>
        </p>

        {/* Earned section */}
        {earned.length > 0 && (
          <div className="space-y-4">
            <h2 className="text-foreground font-semibold text-sm uppercase tracking-wider">Earned</h2>
        ...
        {locked.length > 0 && (
          <div className="space-y-4">
            <h2 className="text-muted-foreground font-semibold text-sm uppercase tracking-wider">Locked</h2>
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-6">
              {locked.map(badge => (
                <BadgeCard key={badge.id} badge={badge} size="md" />
              ))}
