...
export function BadgeUnlockModal({ open, badgeId, onClose }: BadgeUnlockModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent
        showCloseButton
        className="max-w-[calc(100vw-32px)] sm:max-w-sm bg-page p-6 text-center"
        style={{
      ...
      >
        ...
        <p className="text-[10px] font-semibold uppercase tracking-widest mb-1 text-fg-muted">
          Badge Unlocked
        </p>
        <h2 className="text-2xl font-bold mb-1" style={{ color: config.frameColor }}>
          {badge.name}
        </h2>
        <p className="text-sm text-fg-secondary leading-relaxed mb-6">{badge.description}</p>
