...
function CustomTooltip({ active, payload, label }: { active?: boolean; payload?: { value: number }[]; label?: string }) {
  ...
  return (
    <div className="bg-elevated border border-border rounded-xl px-3 py-2">
      ...
      <p className="amount text-accent text-sm font-semibold">{formatGHS(payload[0].value)}</p>
    </div>
  );
}
...
export function WeeklyChart({ data }: WeeklyChartProps) {
  ...
  return (
    ...
    >
      <p className="text-fg-muted text-xs font-medium uppercase tracking-wider mb-4">7-Day Spend</p>
      <div className="h-36">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} barSize={24} margin={{ top: 0, right: 0, bottom: 0, left: -20 }}>
            ...
            />
            <YAxis
              tick={{ fill: 'var(--text-fg-muted)', fontSize: 10 }}
              axisLine={false}
              tickLine={false}
